...
async function handleGetInquiries(request, env) {
  ...
  const [sponsorRes, contactRes] = await Promise.all([
    fetch(`${env.SUPABASE_URL}/rest/v1/sponsor_inquiries?select=*&order=created_at.desc`, { headers }),
    fetch(`${env.SUPABASE_URL}/rest/v1/contact_messages?select=*&order=created_at.desc`, { headers }),
  ]);

  if (!sponsorRes.ok || !contactRes.ok) {
    return new Response('Failed to fetch inquiries from Supabase', { status: 502 });
  }
  ...
  const combined = [
    ...sponsors.map((s) => ({
      ...
      company: s.company,
      email: s.email,
      subject: `Sponsor Inquiry — ${s.company || s.name}`,
      message: s.message,
    ...
    ...contacts.map((c) => ({
      ...
      company: null,
      email: c.email,
      subject: c.subject || 'Contact Message',
      message: c.message,
